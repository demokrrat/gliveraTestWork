...
function dataJson() {
  return gulp.src(paths.dataJson)
...
function serveInit() {
  ...
  gulp.watch(paths.dataJson).on('change', browserSync.reload)
...
exports.serve = gulp.series(css, html, cssVendor, dataJson, jsVendor, babel, serveInit)
...
function copyData() {
  return gulp.src(paths.dataJson)
  //.pipe($.htmlmin({collapseWhitespace: true}))
    .pipe(gulp.dest(paths.build.dataJson))
    .pipe(plugins.size({
      title: 'data'
    }))
}

exports.build = gulp.series(clean, gulp.parallel(copyHtml, copyJs, copyCss, copyImages, copyFonts, copyData))
...
exports.default = gulp.series(css, html, cssVendor, dataJson, jsVendor, babel, serveInit)
